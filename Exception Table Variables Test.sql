...
#Query Function: Grab necessary Exception Table variables from Training Table, where PPIDs between LDAP and Training match up AND *any* exceptions pop up
SELECT T.COURSE_ID, T.COURSE_TITLE, T.DATE_COURSE_COMPLETED, T.EMAIL, T.FIRST_NAME, T.LAST_NAME, T.MEMBER_ID, T.PPID, T.USERNAME, T.TRAINING_SOURCE, T.TRAINING_REC_ID, T.EMPLOYEE_ID_NUMBER
FROM ldap.t_temp_orait_ggc_ldap AS L
INNER JOIN ldap.t_temp_orait_ggc_training_export AS T
ON L.PPID = T.PPID AND (L.FIRST_NAME != T.FIRST_NAME OR L.NETID != T.USERNAME OR L.LAST_NAME != T.LAST_NAME OR L.EMAIL != T.EMAIL);
#Returns about 210 Records

#INSERT INTO exception_table (URID, COURSE_ID, COURSE_TITLE, DATE_COURSE_COMPLETED, EMAIL, FIRST_NAME, LAST_NAME, MEMBER_ID, PPID, USERNAME, TRAINING_SOURCE, TRAINING_REC_ID, EMPLOYEE_ID_NUMBER, Exception_Reason) VALUES (null, T.COURSE_ID, T.COURSE_TITLE, T.DATE_COURSE_COMPLETED, T.EMAIL, T.FIRST_NAME, T.LAST_NAME, T.MEMBER_ID, T.PPID, T.USERNAME, T.TRAINING_SOURCE, T.TRAINING_REC_ID, T.EMPLOYEE_ID_NUMBER, "Exception Found");

#Query Function: Same as last query, but *supposed* to INSERT INTO the exception table. Doesn't work as-is
#INSERT INTO exception_table (URID, COURSE_ID, COURSE_TITLE, DATE_COURSE_COMPLETED, EMAIL, FIRST_NAME, LAST_NAME, MEMBER_ID, PPID, USERNAME, TRAINING_SOURCE, TRAINING_REC_ID, EMPLOYEE_ID_NUMBER, Exception_Reason) VALUES (null, T.COURSE_ID, T.COURSE_TITLE, T.DATE_COURSE_COMPLETED, T.EMAIL, T.FIRST_NAME, T.LAST_NAME, T.MEMBER_ID, T.PPID, T.USERNAME, T.TRAINING_SOURCE, T.TRAINING_REC_ID, T.EMPLOYEE_ID_NUMBER, "Exception Found")
#SELECT T.COURSE_ID, T.COURSE_TITLE, T.DATE_COURSE_COMPLETED, T.EMAIL, T.FIRST_NAME, T.LAST_NAME, T.MEMBER_ID, T.PPID, T.USERNAME, T.TRAINING_SOURCE, T.TRAINING_REC_ID, T.EMPLOYEE_ID_NUMBER
#FROM ldap.t_temp_orait_ggc_ldap AS L
#INNER JOIN ldap.t_temp_orait_ggc_training_export AS T
...
#Query Function: Grab necessary Exception Table variables from Training Table, where FIRST_NAMEs between LDAP and Training match up AND *any* exceptions pop up
SELECT T.COURSE_ID, T.COURSE_TITLE, T.DATE_COURSE_COMPLETED, T.EMAIL, T.FIRST_NAME, T.LAST_NAME, T.MEMBER_ID, T.PPID, T.USERNAME, T.TRAINING_SOURCE, T.TRAINING_REC_ID, T.EMPLOYEE_ID_NUMBER
FROM ldap.t_temp_orait_ggc_ldap AS L
INNER JOIN ldap.t_temp_orait_ggc_training_export AS T
ON L.FIRST_NAME = T.FIRST_NAME AND (L.PPID != T.PPID OR L.NETID != T.USERNAME OR L.LAST_NAME != T.LAST_NAME OR L.EMAIL != T.EMAIL);
#Returns about 534 Records; too broad/duplicate output